...
class ClevrSceneGraphWithPairsDataset(Dataset):
  def __init__(self, vocab, h5_path, image_dir, image_size=(256, 256),
               ...
               clean_repeats=True):
    super(ClevrSceneGraphWithPairsDataset, self).__init__()

    CLEVR_target_dir = os.path.split(image_dir)[0]
    ...
    self.image_source_dir = self.image_dir.replace("target","source") #Azade

    src_h5_path = h5_path.replace("target","source")
  ...
  def __getitem__(self, index):
    ...
    img_source_path = os.path.join(self.image_source_dir, self.image_paths[index].decode("utf-8"))
    # print("target: ", img_path)
    # print("source: ", img_source_path)
    ...
    return image, image_src, objs, objs_src, boxes, boxes_src, triples, triples_src, img_path


def clevr_collate_fn_withpairs(batch):
  """
  ...
  for i, (img, image_src, objs, objs_src, boxes, boxes_src, triples, triples_src, img_path) in enumerate(batch):
  ...
  out = (all_imgs, all_imgs_src, all_objs, all_objs_src, all_boxes, all_boxes_src, all_triples, all_triples_src,
         all_obj_to_img, all_triple_to_img, all_img_paths) #, all_imgs_masked)
